/* Includes ------------------------------------------------------------------*/
#include "IAesGcm.h"
#include <string.h>
#include <stdio.h>
/* Private  defines -----------------------------------------------------------*/

/* AES Private constants ----------------------------------------------------------*/
/* 正向 s-box 变换表 */
static  const sg_uint8_t Sbox[256] = {
};

/* 反向 s-box 变换表 */
static const sg_uint8_t InvSbox[256] = {
};

/* 列混合中的域加/域乘用到的乘数 */
static const sg_uint8_t Rcon[11] = {
0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

/* Xtimes2[Sbox[]] 正向变换中的域乘2，且S-box变换结果表 */
static const sg_uint8_t Xtime2Sbox[256] = {

};

/* Xtimes3[Sbox[]] 正向变换中的域乘3结果表 */
static const sg_uint8_t Xtime3Sbox[256] = {

};

/*
* modular multiplication tables
* Xtime2[x] = (x & 0x80 ? 0x1b : 0) ^ (x + x)
* Xtime3[x] = x^Xtime2[x];
*/

/* 域乘0x09结果表 */
static const sg_uint8_t Xtime9[256] = {
};

/* 域乘0x0b结果表 */
static const sg_uint8_t XtimeB[256] = {
};

/* 域乘0x0d结果表 */
static const sg_uint8_t XtimeD[256] = {
};

/* 域乘0x0e结果表 */
static const sg_uint8_t XtimeE[256] = {
};

/* Private functions ---------------------------------------------------------*/

/* function body */
/*
*	Function:  ShiftRows
*	Action:    S-box变换和行变换
*	Input:     state  - 4*4的状态表首指针
*	Output:    row0 - 位置不变        row1 - 向左循环移动1位
*              row2 - 向左循环移动2位  row3 - 向左循环移动3位
*/
static void ShiftRows(sg_uint8_t *state)
{
	sg_uint8_t tmp;

    /* just substitute row 0 */
    state[0]  = Sbox[state[0]];
    state[4]  = Sbox[state[4]];
    state[8]  = Sbox[state[8]];
    state[12] = Sbox[state[12]];

    /* rotate row 1 */
    tmp       = Sbox[state[1]];
    state[1]  = Sbox[state[5]];
    state[5]  = Sbox[state[9]];
    state[9]  = Sbox[state[13]];
    state[13] = tmp;

    /* rotate row 2 */
    tmp       = Sbox[state[2]];
    state[2]  = Sbox[state[10]];
    state[10] = tmp;
    tmp       = Sbox[state[6]];
    state[6]  = Sbox[state[14]];
    state[14] = tmp;

    /* rotate row 3 */
    tmp       = Sbox[state[15]];
    state[15] = Sbox[state[11]];
    state[11] = Sbox[state[7]];
    state[7]  = Sbox[state[3]];
    state[3]  = tmp;
}

/*
*	Function:  InvShiftRows
*	Action:    反S-box变换和反行变换
*	Input:     state  - 4*4的状态表首指针
*	Output:    row0 - 位置不变         row1 - 向右循环移动1位
*              row2 - 向右循环移动2位  row3 - 向右循环移动3位
*/
static void InvShiftRows(sg_uint8_t *state)
{
	sg_uint8_t tmp;

    /* restore row 0 */
    state[0]  = InvSbox[state[0]];
    state[4]  = InvSbox[state[4]];
    state[8]  = InvSbox[state[8]];
    state[12] = InvSbox[state[12]];

    /* restore row 1 */
    tmp       = InvSbox[state[13]];
    state[13] = InvSbox[state[9]];
    state[9]  = InvSbox[state[5]];
    state[5]  = InvSbox[state[1]];
    state[1]  = tmp;

    /* restore row 2 */
    tmp       = InvSbox[state[2]];
    state[2]  = InvSbox[state[10]];
    state[10] = tmp;
    tmp       = InvSbox[state[6]];
    state[6]  = InvSbox[state[14]];
    state[14] = tmp;

    /* restore row 3 */
    tmp       = InvSbox[state[3]];
    state[3]  = InvSbox[state[7]];
    state[7]  = InvSbox[state[11]];
    state[11] = InvSbox[state[15]];
    state[15] = tmp;
}

/******************************************************************************

* NOTE:     列混合操作。事实上是一种替代操作，用 State 字节列的值进行数学域加
*           和域乘的结果代替每个字节。
*           包括S-box变换，行变换，列变换
*           MixColumns 使用特殊的加法和乘法，是基于近代数学的域论的有限域GF(28).
*           GF(28)由一组从 0x00 到 0xff 的256个值组成，加上加法和乘法。 GF代表
*           伽罗瓦域，以发明这一理论的数学家的名字命名。GF(28) 的一个特性是一个
*           加法或乘法的操作的结果必须是在{0x00 ... 0xff}这组数中。虽然域论是相
*           当深奥的，但GF(28)加法的最终结果却很简单。GF(28) 加法就是异或（XOR）
*           操作。
*
*       　　GF(28)的乘法有点繁难。AES的加密和解密例程需要知道怎样只用七个常量
*           0x01、0x02、0x03、0x09、0x0b、0x0d 和 0x0e 来相乘。只是针对这七种特殊
*           情况进行说明。
*
*       　　在GF(28)中用0x01的乘法是特殊的；它相当于普通算术中用1做乘法并且结果
*           也同样—任何值乘0x01等于其自身。
*
*       　　用0x02做乘法。和加法的情况相同，理论是深奥的，但最终结果十分简单。只要
*           被乘的值小于0x80，这时乘法的结果就是该值左移1比特位。如果被乘的值大于
*           或等于0x80，这时乘法的结果就是左移1比特位再用值0x1b异或。它防止了“域溢
*           出”并保持乘法的乘积在范围以内。
*
*           清楚在GF(28)中用0x02建立了加法和乘法，就可以用任何常量去定义乘法。用
*           0x03做乘法时，你可以将0x03分解为2的幂之和。为了用0x03乘以任意字节b，
*           因为 0x03 = 0x02 + 0x01，因此：
*               b * 0x03 = b * (0x02 + 0x01) = (b * 0x02) + (b * 0x01)
*******************************************************************************/
/*
*	Function:  MixSubColumns
*	Action:    列混合操作,包括S-box变换，行变换，列变换
*	Input:     state  - 4*4的状态表首指针
*	Output:    none
*/
void MixSubColumns(sg_uint8_t *state)
{
	sg_uint8_t newstate[4 * Nc];    //用于缓存新的"state"数组
	sg_uint8_t i;

	newstate[0]  = Xtime2Sbox[state[0]] ^ Xtime3Sbox[state[5]] ^ Sbox[state[10]] ^ Sbox[state[15]];
    newstate[1]  = Sbox[state[0]] ^ Xtime2Sbox[state[5]] ^ Xtime3Sbox[state[10]] ^ Sbox[state[15]];
    newstate[2]  = Sbox[state[0]] ^ Sbox[state[5]] ^ Xtime2Sbox[state[10]] ^ Xtime3Sbox[state[15]];
    newstate[3]  = Xtime3Sbox[state[0]] ^ Sbox[state[5]] ^ Sbox[state[10]] ^ Xtime2Sbox[state[15]];

    /* mixing column 1 */
    newstate[4]  = Xtime2Sbox[state[4]] ^ Xtime3Sbox[state[9]] ^ Sbox[state[14]] ^ Sbox[state[3]];
    newstate[5]  = Sbox[state[4]] ^ Xtime2Sbox[state[9]] ^ Xtime3Sbox[state[14]] ^ Sbox[state[3]];
    newstate[6]  = Sbox[state[4]] ^ Sbox[state[9]] ^ Xtime2Sbox[state[14]] ^ Xtime3Sbox[state[3]];
    newstate[7]  = Xtime3Sbox[state[4]] ^ Sbox[state[9]] ^ Sbox[state[14]] ^ Xtime2Sbox[state[3]];

    /* mixing column 2 */
    newstate[8]  = Xtime2Sbox[state[8]] ^ Xtime3Sbox[state[13]] ^ Sbox[state[2]] ^ Sbox[state[7]];
    newstate[9]  = Sbox[state[8]] ^ Xtime2Sbox[state[13]] ^ Xtime3Sbox[state[2]] ^ Sbox[state[7]];
    newstate[10] = Sbox[state[8]] ^ Sbox[state[13]] ^ Xtime2Sbox[state[2]] ^ Xtime3Sbox[state[7]];
    newstate[11] = Xtime3Sbox[state[8]] ^ Sbox[state[13]] ^ Sbox[state[2]] ^ Xtime2Sbox[state[7]];

    /* mixing column 3 */
    newstate[12] = Xtime2Sbox[state[12]] ^ Xtime3Sbox[state[1]] ^ Sbox[state[6]] ^ Sbox[state[11]];
    newstate[13] = Sbox[state[12]] ^ Xtime2Sbox[state[1]] ^ Xtime3Sbox[state[6]] ^ Sbox[state[11]];
    newstate[14] = Sbox[state[12]] ^ Sbox[state[1]] ^ Xtime2Sbox[state[6]] ^ Xtime3Sbox[state[11]];
    newstate[15] = Xtime3Sbox[state[12]] ^ Sbox[state[1]] ^ Sbox[state[6]] ^ Xtime2Sbox[state[11]];

	for(i=0; i<sizeof(newstate); i++)
	{
		state[i] = newstate[i];
	}
//    memcpy(state, newstate, sizeof(newstate));
}

/*
*	Function:  InvMixSubColumns
*	Action:    反向列混合操作,包括S-box变换，行变换，列变换
*	Input:     state  - 4*4的状态表首指针
*	Output:    none
*/
void InvMixSubColumns(sg_uint8_t *state)
{
	sg_uint8_t  newstate[4 * Nc];   //用于缓存新的"state"数组
	sg_int32_t i;

    /* restore column 0 */
    newstate[0]  = XtimeE[state[0]] ^ XtimeB[state[1]] ^ XtimeD[state[2]] ^ Xtime9[state[3]];
    newstate[5]  = Xtime9[state[0]] ^ XtimeE[state[1]] ^ XtimeB[state[2]] ^ XtimeD[state[3]];
    newstate[10] = XtimeD[state[0]] ^ Xtime9[state[1]] ^ XtimeE[state[2]] ^ XtimeB[state[3]];
    newstate[15] = XtimeB[state[0]] ^ XtimeD[state[1]] ^ Xtime9[state[2]] ^ XtimeE[state[3]];

    /* restore column 1 */
    newstate[4]  = XtimeE[state[4]] ^ XtimeB[state[5]] ^ XtimeD[state[6]] ^ Xtime9[state[7]];
    newstate[9]  = Xtime9[state[4]] ^ XtimeE[state[5]] ^ XtimeB[state[6]] ^ XtimeD[state[7]];
    newstate[14] = XtimeD[state[4]] ^ Xtime9[state[5]] ^ XtimeE[state[6]] ^ XtimeB[state[7]];
    newstate[3]  = XtimeB[state[4]] ^ XtimeD[state[5]] ^ Xtime9[state[6]] ^ XtimeE[state[7]];

    /* restore column 2 */
    newstate[8]  = XtimeE[state[8]] ^ XtimeB[state[9]] ^ XtimeD[state[10]] ^ Xtime9[state[11]];
    newstate[13] = Xtime9[state[8]] ^ XtimeE[state[9]] ^ XtimeB[state[10]] ^ XtimeD[state[11]];
    newstate[2]  = XtimeD[state[8]] ^ Xtime9[state[9]] ^ XtimeE[state[10]] ^ XtimeB[state[11]];
    newstate[7]  = XtimeB[state[8]] ^ XtimeD[state[9]] ^ Xtime9[state[10]] ^ XtimeE[state[11]];

    /* restore column 3 */
    newstate[12] = XtimeE[state[12]] ^ XtimeB[state[13]] ^ XtimeD[state[14]] ^ Xtime9[state[15]];
    newstate[1]  = Xtime9[state[12]] ^ XtimeE[state[13]] ^ XtimeB[state[14]] ^ XtimeD[state[15]];
    newstate[6]  = XtimeD[state[12]] ^ Xtime9[state[13]] ^ XtimeE[state[14]] ^ XtimeB[state[15]];
    newstate[11] = XtimeB[state[12]] ^ XtimeD[state[13]] ^ Xtime9[state[14]] ^ XtimeE[state[15]];

    for (i=0; i<(4 * Nc); i++)
    {
        state[i] = InvSbox[newstate[i]];
    }
}

/*
*	Function:  AddRoundKey
*	Action:    密钥加操作
*	Input:     state - 4*4的状态表首指针
*			   key   - 密钥表首指针
*	Output:    none
*/
static void AddRoundKey(sg_uint8_t *state, sg_uint8_t *key)
{
	sg_int32_t idx;

    for (idx=0; idx<4; idx++)
    {
		state[4*idx+0] ^= key[4*idx+0];
		state[4*idx+1] ^= key[4*idx+1];
		state[4*idx+2] ^= key[4*idx+2];
		state[4*idx+3] ^= key[4*idx+3];
    }
}

/* AES-128-bit Public functions ----------------------------------------------------------*/
/*
*	Function:  AES_ExpandKey
*	Action:    扩展密钥操作，每轮扩展Nk字节
*	Input:     key      - 密钥表首指针
*			   expKey   - 扩展密钥首指针
*	Output:    none
*/
void AES_Expandkey( sg_uint8_t *key, sg_uint8_t *expKey)
{
    sg_uint8_t  tmp0, tmp1, tmp2, tmp3, tmp4;
    sg_int32_t idx;    //索引

    for (idx=0; idx<Nk; idx++)  //把数组key中的每个元素复制到数组expKey相对应中的位置，Nk=4
    {
        expKey[4*idx+0] = key[4 * idx + 0];
        expKey[4*idx+1] = key[4 * idx + 1];
        expKey[4*idx+2] = key[4 * idx + 2];
        expKey[4*idx+3] = key[4 * idx + 3];
    }

    for (idx=Nk; idx<(Nc * (Nr + 1)); idx++)  //
    {
        tmp0 = expKey[4*idx - 4];
        tmp1 = expKey[4*idx - 3];
        tmp2 = expKey[4*idx - 2];
        tmp3 = expKey[4*idx - 1];

        if ( !(idx % Nk) )
        {
            tmp4 = tmp3;
            tmp3 = Sbox[tmp0];
            tmp0 = Sbox[tmp1] ^ Rcon[idx/Nk];
            tmp1 = Sbox[tmp2];
            tmp2 = Sbox[tmp4];
        }

        /* convert from longs to sg_uint8_ts */
        expKey[4*idx+0] = expKey[4*idx - 4*Nk + 0] ^ tmp0;
        expKey[4*idx+1] = expKey[4*idx - 4*Nk + 1] ^ tmp1;
        expKey[4*idx+2] = expKey[4*idx - 4*Nk + 2] ^ tmp2;
        expKey[4*idx+3] = expKey[4*idx - 4*Nk + 3] ^ tmp3;
    }

	for(int i=0;i<176; i++)
	{
		printf("%02x", expKey[i]);
		if((i+1)%4 == 0)
			printf(" ");
		if((i+1)%16 == 0)
			printf("\n");
	}
}

/*
*	Function:  AES_Encrypt
*	Action:    每次加密128Bit数据块
*	Input:     in       -  明文输入缓冲区指针
*              expKey   -  扩展密钥表首指针
*              out      -  密文输出缓冲区指针
*	Output:    密文
*/
void AES_Encrypt( sg_uint8_t *in, sg_uint8_t *expKey, sg_uint8_t *out)
{
	sg_uint8_t  state[Nc * 4];  //state数组，用于数据分组
	sg_int32_t round;          //加密轮数计数器
	sg_int32_t idx;            //索引

    /* 128位数据分组，把输入缓冲区中的数据放到4*4state数组中 */
    for (idx=0; idx<Nc; idx++ )
    {
        state[4*idx+0] = *in++;
        state[4*idx+1] = *in++;
        state[4*idx+2] = *in++;
        state[4*idx+3] = *in++;
    }

    /* 与扩展密钥进行“异或”操作 */
    AddRoundKey ((sg_uint8_t *)state, (sg_uint8_t *)expKey);

    /* Nr轮加密操作 */
    for (round=1; round<(Nr + 1); round++)
    {
        if (round < Nr)
        {
            MixSubColumns(state);   //包含S-box变换，行变换，列变换
        }
        else
        {
            ShiftRows (state);      //S-box变换，行变换，最后一轮不进行列变换
        }

        /* 与扩展密钥进行“异或”操作 */
        AddRoundKey ((sg_uint8_t *)state, (sg_uint8_t *)(expKey + round * Nc *4));
    }

    /* 输出128位加密后的数据 */
    for (idx=0; idx<Nc; idx++)
    {
        *out++ = state[4*idx+0];
        *out++ = state[4*idx+1];
        *out++ = state[4*idx+2];
        *out++ = state[4*idx+3];
    }
}

/*
*	Function:  AES_Decrypt
*	Action:    每次解密128Bit数据块
*	Input:     in       -  密文输入缓冲区指针
*              expKey   -  扩展密钥表首指针
*              out      -  明文输出缓冲区指针
*	Output:    明文
*/
void AES_Decrypt( sg_uint8_t *in, sg_uint8_t *expKey, sg_uint8_t *out)
{
	sg_uint8_t  state[Nc * 4];  //state数组，用于加密数据分组
	sg_int32_t round;          //加密轮数计数器
	sg_int32_t idx;            //索引

    /* 128位加密数据分组 */
    for (idx=0; idx<Nc; idx++)
    {
        state[4*idx+0] = *in++;
        state[4*idx+1] = *in++;
        state[4*idx+2] = *in++;
        state[4*idx+3] = *in++;
    }

    /* 与扩展密钥进行“异或”操作 */
    AddRoundKey (state, expKey + (sg_int32_t)Nr * Nc *4);
    round = Nr;

    //* 反S-box变换，反行变换 */
    InvShiftRows(state);

    while (round--)
    {
        /* 与扩展密钥进行“异或”操作 */
        AddRoundKey((sg_uint8_t *)state, (sg_uint8_t *)(expKey + round * Nc *4));

        if (round)
        {
            /* 反S-box变换，反行变换，反列变换 */
            InvMixSubColumns (state);
        }
    }

    /* 输出128位解密数据 */
    for (idx=0; idx<Nc; idx++ )
    {
        *out++ = state[4*idx+0];
        *out++ = state[4*idx+1];
        *out++ = state[4*idx+2];
        *out++ = state[4*idx+3];
    }
}


/* AES-GCM-128 Private constants ----------------------------------------------------------*/
static const sg_uint8_t mask[] = { 0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01 }; // c = b*a
//static const uint8 poly[] = { 0x00, 0xE1 };

/* AES-GCM-128 Private functions ----------------------------------------------------------*/
/*
*	Function:  ECB_SETUP
*	Action:    AES (Rijndael) 块加密算法初始化
*	Input:     key        -  密钥
*              keylen     -  密钥长度（字节）
*              num_rounds -  加密轮数（参数0表示取默认轮数）
*              skey       -  扩展密钥
*	Output:    成功返回 CRYPT_OK
*/
static sg_uint8_t ECB_SETUP(sg_uint8_t *key, sg_uint8_t keylen, sg_uint8_t num_rounds, sg_uint8_t *skey)
{
    if((key  == NULL) || (skey == NULL)) {return CRYPT_ERROR;}
    if (keylen != 16 && keylen != 24 && keylen != 32) {return CRYPT_ERROR;}

    if (num_rounds != 0
        && num_rounds != (10 + ((keylen / 8) - 2) * 2))
    {
        return CRYPT_ERROR;
    }

	if(keylen != 16) { return CRYPT_ERROR;}
	AES_Expandkey(key, skey);

	return CRYPT_OK;
}

/*
*	Function:  ECB_ENC
*	Action:    AES (Rijndael) 块加密算法（仅加密16字节）
*	Input:     pt         -  明文数据
*              ct         -  密文数据
*              skey       -  扩展密钥
*	Output:    成功返回 CRYPT_OK
*/
static void ECB_ENC(sg_uint8_t *pt, sg_uint8_t *ct, sg_uint8_t *skey)
{
    //if((pt == NULL) || (ct == NULL) || (skey == NULL))
        //return -1;
	AES_Encrypt(pt, skey, ct);
}

/*
*	Function:  ECB_DEC
*	Action:    AES (Rijndael) 块解密算法（仅解密16字节）
*	Input:     ct         -  密文数据
*              pt         -  明文数据
*              skey       -  扩展密钥
*	Output:    成功返回 CRYPT_OK
*/
#if 0
 static void ECB_DEC(const uint8 *ct, uint8 *pt, uint8 *skey)
{
    //if((pt == NULL) || (ct == NULL) || (skey == NULL))
        //return -1;
	AES_Decrypt(ct, skey, pt);
}
#endif

/* right shift */
static void gcm_rightshift(sg_uint8_t *a)
{
   sg_int8_t x;
   for (x = 15; x > 0; x--) {
       a[x] = (a[x]>>1) | ((a[x-1]<<7)&0x80);
   }
   a[0] >>= 1;
}

/*
*	Function:  gcm_gf_mult
*	Action:    GF(2128)域乘
*	Input:     a    -  乘数 1
*              b    -  乘数 2
*              c    -  结果 c = a * b
*	Output:    c
*/
static void gcm_gf_mult(const sg_uint8_t *a, const sg_uint8_t *b, sg_uint8_t *c)
{
   sg_uint8_t Z[16], V[16];
   sg_uint8_t x,y,z;

   memset(Z, 0, 16);
   memcpy(V, a, 16);
   for (x = 0; x < 128; x++) {
       if (b[x>>3] & mask[x&7]) {
          for (y = 0; y < 16; y++) {
              Z[y] ^= V[y];
          }
       }
       z     = V[15] & 0x01;
       gcm_rightshift(V);

       if(z)
       {
            V[0] ^= 0xE1;
       }
   }
   memcpy(c, Z, 16);
}

/*
*	Function:  void gcm_mult_h(gcm_state *gcm, uint8 *I)
*	Action:    GCM multiply by H
*	Input:     gcm    - The GCM state  I - The value to multiply H by
*	Output:    none
*/
static void gcm_mult_h(gcm_state *gcm, sg_uint8_t *I)
{
   sg_uint8_t T[16];

   gcm_gf_mult(gcm->H, I, T);
   memcpy(I, T, 16);
}

/*
*	Function:  int8 gcm_init(gcm_state *gcm, const uint8 *key, uint8 keylen)
*	Action:    Initialize a GCM state
*	Input:     gcm    - The GCM state  key - The secret key
*              keylen - The length of the secret key
*	Output:    CRYPT_OK on success
*/
static sg_int8_t gcm_init(gcm_state *gcm, sg_uint8_t *key, sg_uint8_t keylen)
{
	sg_int8_t  err;
	sg_uint8_t B[16];

	if((gcm == NULL) || (key == NULL)) {return CRYPT_ERROR;;}

	/* schedule key */
	if ((err = ECB_SETUP(key, keylen, 0, gcm->K)) != CRYPT_OK)
	{
		return err;
	}
	/* H = E(0) */
	memset(B, 0, 16);
	ECB_ENC(B, gcm->H, gcm->K);
	/* setup state */
	memset(gcm->buf, 0, sizeof(gcm->buf));
	memset(gcm->X,   0, sizeof(gcm->X));
	gcm->mode     = GCM_MODE_IV;
	gcm->ivmode   = 0;
	gcm->buflen   = 0;
	gcm->totlen   = 0;
	gcm->pttotlen = 0;

	return CRYPT_OK;
}

/*
*	Function:  int8 gcm_reset(gcm_state *gcm)
*	Action:    Reset a GCM state
*	Input:     gcm  - The GCM state
*	Output:    CRYPT_OK on success
*/
static void gcm_reset(gcm_state *gcm)
{
    if(gcm == NULL) { return ;}
	memset(gcm->buf, 0, sizeof(gcm->buf));
	memset(gcm->X,   0, sizeof(gcm->X));
	gcm->mode     = GCM_MODE_IV;
	gcm->ivmode   = 0;
	gcm->buflen   = 0;
	gcm->totlen   = 0;
	gcm->pttotlen = 0;
}

/*
*	Function:  int8 gcm_add_iv(gcm_state *gcm,const uint8 *IV, uint8 IVlen)
*	Action:    Add IV data to the GCM state
*	Input:     gcm  - The GCM state  IV - The initial value data to add
*              IVlen - The length of the IV
*	Output:    CRYPT_OK on success
*/
static void gcm_add_iv(gcm_state *gcm, const sg_uint8_t *IV, sg_uint8_t IVlen)
{
	sg_uint8_t x, y;

   	 if((gcm == NULL) || ((IVlen > 0) && (IV == NULL))) {return;}

	/* must be in IV mode */
	if (gcm->mode != GCM_MODE_IV) { return; }
 	if (gcm->buflen >= 16 || gcm->buflen < 0) { return; }

	/* trip the ivmode flag */
	if (IVlen + gcm->buflen > 12) {gcm->ivmode |= 1;}

	/* start adding IV data to the state */
	for (x = 0; x < IVlen; x++)
	{
		gcm->buf[gcm->buflen++] = *IV++;
		if (gcm->buflen == 16)
		{
			/* GF mult it */
			for (y = 0; y < 16; y++)
			{
				gcm->X[y] ^= gcm->buf[y];
			}
			gcm_mult_h(gcm, gcm->X);
			gcm->buflen = 0;
			gcm->totlen += 128;
		}
	}
	return;
}

/*
*	Function:  int8 gcm_add_aad(gcm_state *gcm,const uint8 *adata, uint16 adatalen)
*	Action:    Add AAD to the GCM state
*	Input:     gcm  - The GCM state  adata - The additional authentication data to add to the GCM state
*              adatalen - The length of the AAD data
*	Output:    CRYPT_OK on success
*/
static void gcm_add_aad(gcm_state *gcm, const sg_uint8_t *adata, sg_int32_t adatalen)
{
	sg_int32_t x;

	if(gcm == NULL) {return;}
	if((adatalen > 0) && (adata == NULL)) {return;}
	if (gcm->buflen > 16 || gcm->buflen < 0) {return;}

	/* in IV mode? */
	if (gcm->mode == GCM_MODE_IV)
	{
		/* let's process the IV */
		if (gcm->ivmode || gcm->buflen != 12)
		{
			for (x = 0; x < (sg_int32_t)gcm->buflen; x++)
			{
				gcm->X[x] ^= gcm->buf[x];
			}
			if (gcm->buflen)
			{
				gcm->totlen += gcm->buflen << 3;
				gcm_mult_h(gcm, gcm->X);
			}
			/* mix in the length */
			memset(gcm->buf, 0, 14);
			STORE16H(gcm->totlen, gcm->buf+14);

			for (x = 0; x < 16; x++)
			{
				gcm->X[x] ^= gcm->buf[x];
			}
			gcm_mult_h(gcm, gcm->X);
			/* copy counter out */
			memcpy(gcm->Y, gcm->X, 16);
			memset(gcm->X, 0, 16);
		}
		else
		{
			memcpy(gcm->Y, gcm->buf, 12);
			gcm->Y[12] = 0;
			gcm->Y[13] = 0;
			gcm->Y[14] = 0;
			gcm->Y[15] = 1;
		}
		memcpy(gcm->Y_0, gcm->Y, 16);
		memset(gcm->buf, 0, 16);
		gcm->buflen = 0;
		gcm->totlen = 0;
		gcm->mode   = GCM_MODE_AAD;
	}
	if (gcm->mode != GCM_MODE_AAD || gcm->buflen >= 16) {return;}

	/* start adding AAD data to the state */
	for (x = 0; x < adatalen; x++)
	{
		gcm->X[gcm->buflen++] ^= *adata++;
		if (gcm->buflen == 16)
		{
			/* GF mult it */
			gcm_mult_h(gcm, gcm->X);
			gcm->buflen = 0;
			gcm->totlen += 128;
		}
		printf("X[%d] = %02x  -   %02x\n", gcm->buflen, gcm->X[gcm->buflen], *adata);
	}
	return;
}

/*
*	Function:  int8 gcm_process(gcm_state *gcm, uint8 *pt, uint16 ptlen, uint8 *ct, int8 direction)
*	Action:    Process plaintext/ciphertext through GCM
*	Input:     gcm  - The GCM state  pt - The plaintext  ptlen - The plaintext length (ciphertext length is the same)
*              ct - The ciphertext  direction - Encrypt or Decrypt mode (GCM_ENCRYPT or GCM_DECRYPT)
*	Output:    CRYPT_OK on success
*/
static void gcm_process(gcm_state *gcm, sg_uint8_t *pt, sg_int32_t ptlen, sg_uint8_t *ct, sg_int8_t direction)
{
	sg_int32_t  x;
	sg_uint8_t   b;
    	sg_int8_t    y;

	if(gcm == NULL) {return;}
	if (ptlen > 0)
	{
        if((pt  == NULL) || (ct  == NULL))
            return;
	}
	if (gcm->buflen > 16 || gcm->buflen < 0) {return;}

	/* in AAD mode? */
	if (gcm->mode == GCM_MODE_AAD)
	{
		/* let's process the AAD */
		if (gcm->buflen)
		{
			gcm->totlen += gcm->buflen << 3;
			gcm_mult_h(gcm, gcm->X);
		}
		/* increment counter */
		for (y = 15; y >= 0; y--)
		{
			if (++gcm->Y[y] & 255)
			{
				break;
			}
		}
		/* encrypt the counter */
		ECB_ENC(gcm->Y, gcm->buf, gcm->K);
		gcm->buflen = 0;
		gcm->mode   = GCM_MODE_TEXT;
	}

	/* process text */
	if (gcm->mode != GCM_MODE_TEXT) {return;}

	for (x = 0; x < ptlen; x++)
	{
		if (gcm->buflen == 16)
		{
			gcm->pttotlen += 128;
			gcm_mult_h(gcm, gcm->X);

			/* increment counter */
			for (y = 15; y >= 0; y--)
			{
				if (++gcm->Y[y] & 255)
				{
					break;
				}
			}
			ECB_ENC(gcm->Y, gcm->buf, gcm->K);
			gcm->buflen = 0;
		}
		if (direction == GCM_ENCRYPT)
		{
			b = ct[x] = pt[x] ^ gcm->buf[gcm->buflen];
		}
		else
		{
			b = ct[x];
			pt[x] = ct[x] ^ gcm->buf[gcm->buflen];
		}
		gcm->X[gcm->buflen++] ^= b;
	}

	printf("\n-------------C--------------\n");
	for(int i=0;i<16;i++)
	{
		printf("%02x ", ct[i]);
	}
	printf("\n----------------------------\n");

	return;
}

/*
*	Function:  int8 gcm_done(gcm_state *gcm, int8 *tag, uint8 *taglen)
*	Action:    Terminate a GCM stream
*	Input:     gcm  - The GCM state  tag - The destination for the MAC tag  taglen - The length of the MAC tag
*	Output:    CRYPT_OK on success
*/
static int gcm_done(gcm_state *gcm, sg_uint8_t *tag, sg_uint8_t *taglen , sg_uint8_t dir)
{
	sg_uint8_t x;
	sg_uint8_t t[128] = {0};

	if (gcm->buflen > 16 || gcm->buflen < 0) {return -1;}

	if (gcm->mode != GCM_MODE_TEXT) {return -1;}
	/* handle remaining ciphertext */
	if (gcm->buflen)
	{
        gcm->pttotlen += ((gcm->buflen) << 3);
		gcm_mult_h(gcm, gcm->X);
	}
	/* length */
	memset(gcm->buf, 0, 6);
	STORE16H(gcm->totlen, gcm->buf+6);
	memset(gcm->buf+8, 0, 6);
	STORE16H(gcm->pttotlen, gcm->buf+14);

	for (x = 0; x < 16; x++)
	{
		gcm->X[x] ^= gcm->buf[x];
	}
	gcm_mult_h(gcm, gcm->X);

	/* encrypt original counter */
	ECB_ENC(gcm->Y_0, gcm->buf, gcm->K);

	for (x = 0; x < 16 && x < *taglen; x++)
	{
		if(dir == GCM_ENCRYPT)
			tag[x] = gcm->buf[x] ^ gcm->X[x];
		else
		{
			t[x] =	gcm->buf[x] ^ gcm->X[x];
			if(t[x] != tag[x])
			{
				printf("tag error %02x %02x",t[x], tag[x]);
			}
		}
	}
	*taglen = x;

	return 0;
}

/* AES-GCM-128 Public functions ------------------------------------------*/
void gcm_packet(sg_uint8_t *key, sg_uint8_t keyLen, sg_uint8_t *dt, sg_int32_t dtLen, sg_uint8_t *iv, sg_uint8_t ivLen, sg_uint8_t *aad, sg_int32_t aadLen, sg_uint8_t *tag, sg_uint8_t tagLen, sg_uint8_t dir)
{
    gcm_state gcm;
    if( (key == NULL)
        || (keyLen != 16)
        || (dtLen && (dt == NULL))
        || (ivLen && (iv == NULL))
        || (aadLen && (aad == NULL))
        || (tagLen && (tag == NULL))
        || (gcm_init(&gcm, key, keyLen) != CRYPT_OK)) {return;}

    gcm_reset(&gcm);                              // reset the state

	sg_uint8_t	cliper[128] = {0};
	sg_uint8_t	ivTmp[16] = {0};
	memcpy(ivTmp, iv, 12);
	ivTmp[15] = 0x2;
	AES_Encrypt( ivTmp, key, cliper);
	for(int i=0;i<16;i++)
	{
		printf("%02x ", cliper[i]);
	}
	printf("\n----------------------------\n");

    gcm_add_iv(&gcm, iv, ivLen);                  // Add the IV
    gcm_add_aad(&gcm, aad, aadLen);               // Add the AAD (note: aad can be NULL if aadlen == 0)
    gcm_process(&gcm, dt, dtLen, dt, dir);        // process the plaintext or cryptograph
    gcm_done(&gcm, tag, &tagLen, dir);         // Finish up and get the MAC tag
}
